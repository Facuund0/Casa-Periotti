...
export async function assertSaleFiscalChoice(
  ...
  const [threshold, total] = await Promise.all([
    ...
    estimateOrderTotal(adminDb, sale.customerId, sale.items, sale.pricePreference),
  ]);
...
/**
 * Total que va a calcular create_order() (migraciones 0006 y 0019): la
 * regla de precio de wholesale-pricing.ts (tipo de cliente, preferencia
 * y mínimo por producto) y suma de round(precio × cantidad, 2). Es solo
 * para decidir si hace falta identificar al comprador; el total que vale
 * es el del pedido.
 */
async function estimateOrderTotal(
  ...
  let customerType: CustomerType = "minorista";
  if (customerId) {
    const { data } = await adminDb
      ...
      .eq("id", customerId)
      .maybeSingle();
    customerType = (data?.customer_type as CustomerType) ?? "minorista";
  }
  ...
  return items.reduce((sum, item) => {
    ...
    const { unitPrice } = resolveLinePrice(
      {
      ...
      customerType,
      pricePreference,
      totals.get(item.productId) ?? item.quantity
    );
    return sum + Math.round(unitPrice * item.quantity * 100) / 100;
  }, 0);
}
